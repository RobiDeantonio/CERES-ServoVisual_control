...
def DistanciaGripper(paquete):
    pub = rospy.Publisher("DistanciaGripper", Float32, queue_size=10)
    rospy.init_node("CamaraPrimeSense",anonymous=True)#Se inicia el nodo ACTUADORESPY(Actuadores python)
    rate = rospy.Rate(10) #10 Hz
    if not rospy.is_shutdown():
        hello_str = paquete
...
while not done:
    ...
    print('Center pixel is {} mm away'.format(dmap[YO,XO]))
    ...
    if int(y + h / 2) > 239:
        ...
        YO = int(y1 + h / 2)
    DistanciaGripper([XO, YO, dmap[YO, XO]])  # Se esta vectorizando
    print('Center pixel is {} mm away'.format(dmap[YO, XO]))
    print('Pixel X:' + str(YO) + ' Y:' + str(XO))
    ...
    getContours(imgDil, rgb)
    #Dibujar poscición objeto de interes
    cv2.rectangle(rgb, (x1, y1), (x1 + w, y1 + h), (0, 0, 255), 3)
    cv2.circle(rgb, (int(x1 + w / 2), int(y1 + h / 2)), 5, (0, 0, 255))
    ...
    # hStack = np.hstack([rgb, mask, result])
    ...
    cv2.imshow('ColorNegro', mascara1)
# end while
